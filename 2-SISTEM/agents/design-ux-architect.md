...
| **Tetikleyici** | Her FE task (UI Designer'dan sonra); proje başı global tasarım sistemi |
| **Sıra** | FE pipeline **adım 2** (asla atlanmaz) |
| **Önceki** | `@kadikoy-ui-designer` |
| **Sonraki** | `@kadikoy-frontend-developer` (implementasyon brief'i ile görevlendirir) |
...
- Grid **kırılabilir** — elementler grid'i aşabilir
- **Asimetri** ve **overlap** (kart, görsel, tipografi örtüşmesi) derinlik verir
- Negative space korkusuz kullan

### Animasyon spesifikasyonu (Frontend Developer'a aktar)
```css
--ease-smooth: cubic-bezier(0.4, 0, 0.2, 1);
--ease-spring: cubic-bezier(0.34, 1.56, 0.64, 1);
--ease-sharp: cubic-bezier(0.76, 0, 0.24, 1);
```
- Sayfa girişi: staggered fade-up
- Hover: `transform + opacity` (asla sadece renk)
- Scroll: IntersectionObserver ile tetikle
- Süre: 200–400ms (landing); dashboard'da 200–300ms (hızlı, keskin)
...
- `<1024px`: sidebar off-canvas; iki kolon → tek kolon
- `<768px`: padding küçült; H1/H2 ölçekle; butonlar dikey stack
- Mobilde ağır blur/glow kapat veya hafiflet; touch target min 44px

### Frontend Developer görevlendirme brief'i (zorunlu çıktı)
```markdown
## UX Implementasyon Brief
**Estetik kaynak:** [UI Designer vizyon bloğu]
**Dosya yapısı:** css/design-system.css, layout.css, components.css, theme-manager.js
**Öncelik:** 1) CSS variables + dual theme 2) Layout 3) Components 4) Motion 5) Content
**Angular:** standalone, signals, @if/@for, SCSS scoped + global tema
**Yasak:** Generic AI slop, Bootstrap/MUI default, yasak fontlar
```
...
- Provide clear handoff specifications for Frontend Developer
- Ensure Forma estetik vizyonu teknik temele dönüştürülmeden kod aşamasına geçilmez
...
**Developer Handoff**: Ready for Frontend Developer implementation (Forma brief attached)
